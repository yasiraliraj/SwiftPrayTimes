...
public struct PrayTimes {
    ...
    public struct PrayerResult {
        ...
        public init(
            ...
            timeSuffixes: [String]? = nil) {
                ...
                if self.time > 24 {
                    // Increment day
                    self.date = (Calendar.current as NSCalendar)
                        .date(byAdding: .day,
                            value: 1,
                            to: self.date,
                ...
                case .dhuhr:
                    ...
                    let flags = NSCalendar.Unit.weekday
    ...
    public mutating func getTimes(
        ...
        handler: @escaping (_ prayers: [PrayerResult]) -> Void) {
            lat = coordinates[0]
            lng = coordinates[1]
            ...
            let deferredTask: () -> Void = {
                var result = selfCopy.computeTimes().map {
                ...
                for (index, item) in result.enumerated() {
                    if item.date.compare(date) == .orderedAscending {
                        if !item.isCurrent {
                            result[index].date = (Calendar.current as NSCalendar)
                                .date(byAdding: .day,
                                    value: 1,
            ...
            if let tz = timeZone {
                ...
                self.timeZone = Double(NSTimeZone.local.secondsFromGMT()) / 60.0 / 60.0
                ...
                URLSession.shared.dataTask(with: URL(string: url)!, completionHandler: {
                    ...
                    if (error == nil) {
                        ...
                        if (err == nil && (jsonData["status"] as? String) == "OK") {
                            selfCopy.timeZone = jsonData["rawOffset"] as! Double / 60.0 / 60.0
                            
                            // Factor in daylight if applicable
                            if dst {
                                selfCopy.timeZone += (jsonData["dstOffset"] as! Double / 60.0 / 60.0)
                            }
    ...
    public mutating func getTimeSeries(
        ...
        isLocalCoords: Bool = true, // Should set to false if coordinate in parameter not device
        onlyEssentials: Bool = false,
        handler: @escaping (_ series: [PrayerResultSeries]) -> Void) {
    ...
    public mutating func getTimeline(
        ...
        isLocalCoords: Bool = true, // Should set to false if coordinate in parameter not device
        onlyEssentials: Bool = false,
        handlerPerDate: ((_ date: Date, _ prayers: [PrayerResult]) -> Void)? = nil,
        handler: @escaping (_ prayers: [PrayerResult]) -> Void) {
                ...
                getTimes(coordinates,
                    ...
                    onlyEssentials: onlyEssentials) { prayers in
                        ...
                        if date.compare(startOfEndDate) == .orderedSame {
                            ...
                            repeatTask((Calendar.current as NSCalendar)
                                .date(byAdding: .day,
                                    value: 1,
                                    to: date,
                                    options: NSCalendar.Options(rawValue: 0)
    ...
    func computePrayerTimes(_ times: [TimeName: Double]) -> [TimeName: Double] {
    ...
    func sunAngleTime(_ angle: Double!, time: Double!, direction: String? = nil) -> Double {
    ...
    // Adjust times
    func adjustTimes(_ times: [TimeName: Double]) -> [TimeName: Double] {
        var times = times
    ...
    func adjustHighLats(_ times: [TimeName: Double]) -> [TimeName: Double] {
        var times = times
    ...
    // Adjust times for locations in higher latitudes
    func adjustHLTime(_ time: Double!, base: Double!, angle: Double!, night: Double!, direction: String? = nil) -> Double {
        var time = time
    ...
    func nightPortion(_ angle: Double!, _ night: Double!) -> Double {
    ...
    func tuneTimes(_ times: [TimeName: Double]) -> [TimeName: Double] {
        var times = times
    ...
    // Compute asr time
    func asrTime(_ time: Double!) -> Double {
    ...
    func asrFactor(_ asrParam: AdjustmentParam) -> Double {
    ...
    static func dayPortion(_ times: [TimeName: Double]) -> [TimeName: Double] {
        var times = times
    ...
    static func getJulian(_ date: Date) -> Double {
    ...
    static func timeToDecimal(_ date: Date = Date()) -> Double {
        let calendar = Calendar.current
        let components = (calendar as NSCalendar).components([.hour, .minute],
            from: date)
